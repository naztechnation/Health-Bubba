  ...
  Widget build(BuildContext context) {
    return BlocConsumer<UserCubit, UserStates>(listener: (context, state) {
      if (state is UserDataLoaded) {
        ...
        if (state.doctorsAnalytics.ok ?? false) {
          totalConsultation =
              state.doctorsAnalytics.data?.consultations.toString() ?? '0';
          totalPrescription =
              state.doctorsAnalytics.data?.prescriptions.toString() ?? '0';
          totalRevenue = state.doctorsAnalytics.data?.revenue.toString() ?? '0';
          patientDemography =
              "${state.doctorsAnalytics.data?.male.toString()}/${state.doctorsAnalytics.data?.female.toString()}" ??
                  '0/0';
        } else {
          ToastService().showToast(context,
              leadingIcon: const ImageView.svg(AppImages.error),
              title: 'Error!!!',
              subtitle: state.doctorsAnalytics.message ?? '');
        }
